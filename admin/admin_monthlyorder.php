...
<head>
  ...
  <title>Admin | Monthly Order List</title>
                ...
                <div class="callout callout-primary shadow rounded-0">
                    <form action="" id="filter">
                        <div class="row align-items-end">
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-12">
                                <div class="form-group">
                                    <button class="btn btn-success btn-flat btn-sm"><i class="fa fa-filter" <?php if ($role != 1) echo 'disabled'; ?>></i> Filter</button>
                                    <button class="btn btn-light border btn-flat btn-sm" type="button" id="print"><i class="fa fa-print" <?php if ($role != 1) echo 'disabled'; ?>></i> Print</button>
                                </div>
                            </div>
                        </div>
                    </form>
                    ...
                    <table class="table table-bordered table-stripped" id="monthlyorder">
                        ...
                        <tbody>
                            ...
                            $i = 1;
                            $total = 0;
                            $orders = $conn->query("SELECT o.*, o.order_code as ordercode, CONCAT(c.last_name, ', ', c.first_name, ' ', COALESCE(c.middle_name, '')) as buyer FROM `order_list` o INNER JOIN buyer_accounts c ON o.buyer_id = c.id  AND DATE_FORMAT(o.date_created, '%Y-%m') = '$month' ORDER BY UNIX_TIMESTAMP(o.date_created) DESC ");
